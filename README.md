...
├── shared-video-exts.js# 媒体扩展名单一来源 (视频 19 种 + 音频 6 种，主进程过滤与渲染拖拽判断共用)
├── index.html          # 视图模板 (极简语义化结构、CSP 安全策略、毛玻璃 CSS + 音乐效果层)
├── renderer.js         # 渲染进程核心控制器 (手势拖拽、字幕引擎、播放状态机、频谱可视化)
├── build/              # 应用图标与静态构建资源（兼作系统托盘图标）
