...
### 1. 极致纯粹的视觉与交互
- **🌗 浅色 / 深色双外观一键切换**：循环播放键左侧【外观】键即时切换两套主题；品牌青蓝霓虹渐变共享，文字与控件颜色随主题自动反转保证清晰度，选择自动记忆。
- **🪟 全透明顶部状态栏**：无毛玻璃底色的纯净标题栏，仅保留文字与窗口按键；自动字影保证任意画面背景下可读。
...
- **🏠 系统托盘常驻**：主窗口点"关闭"隐藏到托盘继续后台播放；托盘左键单击弹菜单、双击直接打开、右键弹菜单；音乐最小化/隐藏**不暂停后台续播**，视频最小化/隐藏自动暂停（恢复续播）。
- **多维度画面调节**：
...
```
BBPlayer/
├── main.js             # Electron 主进程 (窗口管理、系统托盘、IPC 路由、原生 AspectRatio 锁定)
├── preload.js          # 上下文隔离桥接 (ContextIsolation, 暴露安全受限的 window.electronAPI)
├── shared-video-exts.js# 媒体扩展名单一事实来源 (结构化 video/audio/all，主进程过滤与渲染拖拽判断共用)
├── verify-exts.js      # 扩展名一致性对账脚本 (shared-video-exts ↔ package.json，node verify-exts.js)
├── index.html          # 视图模板 (极简语义化结构、CSP 安全策略、双主题 CSS 变量 + 音乐效果层)
├── renderer.js         # 渲染进程核心控制器 (手势拖拽、字幕引擎、播放状态机、频谱可视化、外观切换)
├── build/              # 应用图标与静态构建资源（彩色播放键：窗口/托盘/安装包统一使用）
