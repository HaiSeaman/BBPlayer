...
软件采用 **"安静玻璃"低占用设计语言**：无 OS 级拖拽区、幽灵图标控制栏、唯一实心强调，
...
- **"安静玻璃"低占用视觉**：界面全面减负——幽灵图标按键、唯一实心播放键、四档统一圆角；模糊与发光收敛到最少，CPU / GPU 占用极低。
...
├── build/              # 应用图标与静态构建资源（彩色播放键：窗口/安装包统一使用）
...
|---|---|---|---|
| **视觉颜值** | 现代极简、"安静玻璃"低占用设计、贴角窗口键、唯一实心强调 | 传统 Windows 面板，UI 繁琐陈旧 | 扁平粗糙，缺乏深度视觉 |
